...
<div class="row">
        ...
        <div class="p-0 pe-md-0 ">
             ...
             <?php  } else { ?>
                    <img class="company_image w-100 shadow" src='<?php echo $company_profile ;?>' alt='image'>
             <?php } ?>
...
<div class="row">
          ...
          <div class="p-0 pe-md-0 ">
               ...
               <?php  } else { ?>
                      <img class="ceo_image w-100 shadow" src='<?php echo $ceo_profile ;?>' alt='image'>
               <?php } ?>
...
<div class="list">
  ...
  $query = mysqli_query($con,"SELECT company.id,
                                    ...
                                    WHERE (job.date_ended >= '$date' and job.job_status = 'Active') and company.status = 'Active'
                                    and job.id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) and company.id = '".$_REQUEST['id']."' group BY job.job_title order by job.id desc ");
...
<div class="row">
  ...
  $query = mysqli_query($con, "SELECT count(count) as job_count, date_ended FROM job WHERE date_ended >= '$date' and id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) and  company_id = '".$_REQUEST['id']."' order by job.id desc");
  while($row = mysqli_fetch_array($query)){ 
  ...
  WHERE (job.date_ended >= '$date' and job.job_status = 'Active') and company.status = 'Active' and job.id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) and company.id = '".$_REQUEST['id']."' group by job.job_category  ");
...
<div class="info-horizontal border-radius-xl d-block d-block p-0 h-100 " style="background: none ;">
 ...
 <?php } } ?> 
  ...
  WHERE (date_ended >= '$date' and job_status = 'Active') and id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) and company_id = '".$_REQUEST['id']."' order by job.id desc");
  while($row = mysqli_fetch_array($query)){ 
...
<!-- END -->
  ...
  WHERE (date_ended >= '$date' and job_status = 'Active') and id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) order by job.id desc");
  while($row = mysqli_fetch_array($query)){ 
