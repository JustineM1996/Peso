...
WHERE (job.date_ended >= '$date' and job.job_status = 'Active') and company.status = 'Active' and job.id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) order by job.id desc");
while($row = mysqli_fetch_array($query)){ 
...
<div class="pagination mt-3"></div>
  ...
  $query = mysqli_query($con, "SELECT count(count) as job_count, date_ended FROM job WHERE date_ended >= '$date' and id NOT IN ( '" . implode( "', '" , $job_hiring ) . "' ) order by job.id desc");
  while($row = mysqli_fetch_array($query)){ 
